...
$$SPD = P(\hat{Y} = 1 \mid D = \text{unprivileged}) - P(\hat{Y} = 1 \mid D = \text{privileged})$$

**Disparate Impact (DI)**

$$DI = \frac{P(\hat{Y} = 1 \mid D = \text{unprivileged})}{P(\hat{Y} = 1 \mid D = \text{privileged})}$$
...
$$AAOD = \frac{(TPR_{\text{Female}} - TPR_{\text{Male}}) + (FPR_{\text{Female}} - FPR_{\text{Male}})}{2}$$

**Fairness Impact Score (FIS)** — our composite metric

$$FIS = \frac{1}{2} \cdot \frac{|SPD| + |1 - DI|}{2} + \frac{1}{2} \cdot \frac{|EOD| + |AAOD|}{2}$$
